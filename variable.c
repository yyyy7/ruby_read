...
classname(VALUE klass, int *permanent)
{
    ...
    *permanent = 0;
    ...
    if (st_lookup(ivtbl, (st_data_t)tmp_classpath, &n))
        return (VALUE)n;
    return Qnil;
}
...
void rb_define_virtual_variable(
    const char *name,
    rb_gvar_getter_t *getter,
    rb_gvar_setter_t *setter)
{
...
rb_f_untrace_var(int argc, const VALUE *argv)
{
    ...
    if (NIL_P(cmd))
    {
        ...
        if (!entry->var->block_trace)
            remove_trace(entry->var);
        return ary;
    }
    else
    {
...
{
    if (entry->var->getter == rb_gvar_undef_getter)
        return Qfalse;
    return Qtrue;
}
...
rb_f_global_variables(void)
{
    ...
    if (!NIL_P(backref))
    {
        ...
        for (i = 1; i <= nmatch; ++i)
        {
            if (!rb_match_nth_defined(i, backref))
                continue;
            if (i < 10)
            {
...
generic_ivar_remove(VALUE obj, ID id, VALUE *valp)
{
    ...
    if (!iv_index_tbl)
        return 0;
    if (!st_lookup(iv_index_tbl, key, &index))
        return 0;
    if (!gen_ivtbl_get(obj, &ivtbl))
        return 0;
...
gen_ivar_each(VALUE obj, rb_ivar_foreach_callback_func *func, st_data_t arg)
{
    ...
    if (!iv_index_tbl)
        return;
    if (!gen_ivtbl_get(obj, &data.ivtbl))
        return;
...
void rb_autoload_str(VALUE mod, ID id, VALUE file)
{
    ...
    {
        if (!tbl)
            tbl = RCLASS_IV_TBL(mod) = st_init_numtable();
...
rb_const_search_from(VALUE klass, ID id, int exclude, int recurse, int visibility)
{
    ...
    while (RTEST(tmp))
    {
        ...
        while ((ce = rb_const_lookup(tmp, id)))
        {
            ...
            {
                if (BUILTIN_TYPE(tmp) == T_ICLASS)
                    tmp = RBASIC(tmp)->klass;
...
rb_const_list(void *data)
{
    ...
    return ary; /* ary element is T_SYMBOL */
}
...
void rb_const_set(VALUE klass, ID id, VALUE val)
{
    ...
    {
        struct autoload_const ac = {
            .mod = klass, .id = id, .value = val, .flag = CONST_PUBLIC,
